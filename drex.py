...
def train_reward(args):
    ...
    for spec, costs in zip(env_kwargs, [[0.01, 0.008], [0.1, 0.17]]):
        for cost in costs:    
            ...
            frinction_name = format_name_string(str(spec))
            cost_name = format_name_cost(str(cost))
            noise_injected_traj = f'./log/drex/hopper/noisy_{frinction_name}_penalty_{cost_name}/prebuilt.pkl'
            loaded = dataset.load_prebuilt(noise_injected_trajs)
            assert loaded
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_train_envs', default=4, type=int, help='number of intervened training environments to train on')
