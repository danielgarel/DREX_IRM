        ...
        for _ in tqdm(range(num_samples)):
            ...
            if include_action:
                D.append((np.concatenate((x_traj[0][x_slice], x_traj[1][x_slice]), axis=1),
                          np.concatenate((y_traj[0][y_slice], y_traj[1][y_slice]), axis=1),
                          0 if self.trajs[x_idx][0] < self.trajs[y_idx][0] else 1)
                         # if noise level is small, then it is better traj.
                         )
            else:
                D.append((x_traj[0][x_slice],
                          y_traj[0][y_slice],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env_id', required=True, help='Select the environment to run')
    #     parser.add_argument('--bc_agent',required=True, 'We will instead use PPO policy used to generate suboptimal demo')
    parser.add_argument('--demo_trajs', required=False,
                        help='suboptimal demo trajectories used for bc (used to generate a figure)')
    parser.add_argument('--demo_trajs_dir', required=False,
                        help='suboptimal demo trajectories directories - load all files')
    parser.add_argument('--ctrl_cost', required=False, help='Select the environment to run', default=0.001, type=float)
